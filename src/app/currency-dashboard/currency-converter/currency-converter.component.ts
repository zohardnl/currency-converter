...
export class CurrencyConverterComponent implements OnInit, AfterViewInit {
  ...
  registerValueChanges() {
    this.formGroup.valueChanges.pipe(
      ...
      switchMap((formValue: Record<string, any>) => this.getConversionRateResults(formValue.amount)),
      untilDestroyed(this)
    ).subscribe();
  }

  getConversionRateResults(amount: number): Observable<never> | Observable<ConversionRate> {
    let response$: Observable<never> | Observable<ConversionRate> = EMPTY.pipe(ignoreElements());
    ...
    if (!amount) {
      ...
      if (from === to) {
        ...
        response$ = this.converterService.getConversionRate(this._params).pipe(
